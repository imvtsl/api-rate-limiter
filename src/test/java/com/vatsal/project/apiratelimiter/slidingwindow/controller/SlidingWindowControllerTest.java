...
class SlidingWindowControllerTest {
    private static final String BASE_URL = "/slidingwindow/v1";
    ...
    void testSubmitRequest() throws Exception {
        ...
        Mockito.when(slidingWindowService.submitRequest(Mockito.anyString())).thenReturn(statusResponse);
    ...
    @Test
    void testSubmitRequestTooManyRequests() throws Exception {
        StatusResponse statusResponse = new StatusResponse(false);
        Mockito.when(slidingWindowService.submitRequest(Mockito.anyString())).thenReturn(statusResponse);
        ...
        String response = mvcResult.getResponse().getContentAsString(StandardCharsets.UTF_8);
        JSONAssert.assertEquals(
                "{\"status\":false}", response, JSONCompareMode.LENIENT);
    }

    @Test
    void testDeregisterUser() throws Exception {
        StatusResponse statusResponse = new StatusResponse(true);
        Mockito.when(slidingWindowService.deregisterUser(Mockito.anyString())).thenReturn(statusResponse);
